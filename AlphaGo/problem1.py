...
class TicTacToe(BoardGame):
    ...
    def check_game(self,s):
        ...
        for i in range(np.size(s.b, axis=0)):
            # check column, row, diagonal topleft-bottomright, and the other diagonal in that order for each player
            if np.sum(s.b[:, i]) == 3 or np.sum(s.b[i, :]) == 3 or s.b[0, 0]+s.b[1, 1]+s.b[2, 2] == 3 or s.b[2, 0] + s.b[1, 1] + s.b[0, 2] == 3:
                e = 1
            elif np.sum(s.b[:, i]) == -3 or np.sum(s.b[i, :]) == -3 or s.b[0, 0]+s.b[1, 1]+s.b[2, 2] == -3 or s.b[2, 0] + s.b[1, 1] + s.b[0, 2] == -3:
                e = -1

        if e == 5 and np.count_nonzero(s.b) == 9:
            e = 0
...
class MiniMaxPlayer(Player):
    ...
    def choose_optimal_move(self,n):
        ...
        for i in n.c:
            temp.append(i.v)
        if n.s.x == 1:
            index = np.where(temp == np.amax(temp))[0][0]
        if n.s.x == -1:
